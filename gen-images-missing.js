...
function apiRequest(method, urlPath, body, retries = 3) {
  return new Promise((resolve, reject) => {
    const attempt = () => {
      const opts = {
        hostname: API_HOST, path: urlPath, method: method,
      ...
      req.on('error', (e) => { if (retries > 0) { retries--; setTimeout(attempt, 5000); } else reject(e); });
...
async function main() {
  console.log(`=== Generating ${TASKS.length} missing images with seedream-4.0 ===\n`);

  // Submit one by one with delay to avoid rate limits
  const results = [];
  for (const task of TASKS) {
    for (let retry = 0; retry < 3; retry++) {
      try {
        const res = await apiRequest('POST', '/open-api/v1/image/generate', {
          prompt: task.prompt, model: 'mj-v7', aspectRatio: task.ar
        });
        const tid = res.data?.taskId;
        if (tid) {
          console.log(`[OK] ${task.name} -> task ${tid}`);
          results.push({ ...task, taskId: tid });
          break;
        }
        console.log(`[FAIL] ${task.name} -> ${res.message}`);
        if (retry < 2) await sleep(5000);
      } catch (e) {
        console.log(`[RETRY ${retry}] ${task.name} -> ${e.message.slice(0, 60)}`);
        await sleep(5000);
      }
    }
    await sleep(2000);
    ...
    for (const job of pending) {
      try {
        ...
        if (res.data?.status === 'ready' && res.data?.images?.[0]) {
          const ext = res.data.images[0].split('.').pop().split('?')[0] || 'jpg';
          await download(res.data.images[0], path.join(BASE, 'images', `${job.name}.${ext}`));
          job.done = true;
